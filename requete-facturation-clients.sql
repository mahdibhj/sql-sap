...
SELECT DocEntry, DocTotal, CardCode, CardName, FatherCard,CntctCode,GroupNum,PayToCode,Address,Address2,TrnspCode,DocNum,DocDate,NumAtCard, VatSum, U_FCSDK_ROUTE  FROM OINV  --WHERE DocEntry = ''
...
SELECT DocEntry,'' BaseEntry,'' ItemCode, null INV1_Qty,null Price,'' WhsCode,LineText as ItemDescription,null LineTotal, null LineNum, 'INV10' as 'Table'  FROM INV10 --WHERE DocEntry = ''
...
SELECT DocEntry, LineNum, SUM(Quantity) SUM_Quantity_INV1, SUM(LineTotal) SUM_LineTotal_Item FROM INV1 --WHERE DocEntry = ''
